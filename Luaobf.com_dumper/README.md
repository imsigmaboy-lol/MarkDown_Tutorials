...
```
## Secondly
go to the directory of the obsufcated script you want to dump. example:  if you wanted to dump the bytecode of a file named "obsufcated.luau" and that was in the path "/storage/emulated/0" you wkould go to "/storage/emulated/0/" using 
```bash
...
## OPTIONAL
if u wanna simplify usage in t1he CLI, you can use the flag -i OR -input which tells python what the obsufcated input is. Then -o  OR -output tells python what the output file should be from the input. You can also combine theses if you want to
EXAMPLE:
```python
